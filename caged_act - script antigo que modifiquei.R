...
pasta.txt <- "D:/Bases de dados/caged_act/" # substituir pelo caminho da pasta no computador (manter a barra no final do caminho)
...
## aqui começa minha brincadeira
library(dplyr)

caged <- mutate(caged, ano = year(mes))
  
caged %>% 
  filter(ano != 2022) %>% 
  group_by(ano, act) %>% 
  summarize(soma = sum(saldo, na.rm = TRUE)) %>% 
  ggplot() + geom_col(aes(x = ano, y = soma)) +
  facet_wrap(~act)
